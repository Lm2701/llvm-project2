...
char ::X86TypeSys::ID = 0;

FunctionPass *llvm::createX86DfenceTypeSystemPass() {
    return new X86TypeSys();
}
...
bool X86TypeSys::runOnMachineFunction(MachineFunction &MF) {
    const TargetInstrInfo *TII = MF.getSubtarget().getInstrInfo();
    for (auto &MBB : MF) {
      for (auto MI = MBB.begin(), E = MBB.end(); MI != E; ++MI) {
        if (MI->getOpcode() == X86::RET) {
          DebugLoc DL = MI->getDebugLoc();
          BuildMI(MBB, MI, DL, TII->get(X86::SRET));
